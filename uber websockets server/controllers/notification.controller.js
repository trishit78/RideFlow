...
export async function notifyDriversHandler(req,res) {
    try {
    ...
    res.status(200).send({data:{rideId,...result},success:true,
    error:null,message:"Successfully notified drivers"});

    
} catch (error) {
    res.status(500).send({data:null,success:false,
        error:error.message,message:"failed to notify drivers"});
           
}
}


export async function removeRideNotificationHandler(req, res) {
